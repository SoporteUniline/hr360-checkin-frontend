...
export default function MiSuscripcionPage() {
    ...
    try {
      ...
      enqueueSnackbar(
        err.response?.data?.message ||
          "Error al generar el enlace de reactivación.",
        { variant: "error" },
      );
    } finally {
  ...
  const estadoRaw =
    suscripcion.estado_suscripcion || suscripcion.estado_contrato;
  ...
  const facturasPagadas =
    stripeInfo?.facturas?.filter((f) => f.estado === "paid") || [];
  const facturasAbiertas =
    stripeInfo?.facturas?.filter((f) => f.estado === "open") || [];
  const mesesPagados = facturasPagadas.length;

  // Trial
  const esTrial =
    stripeInfo?.estado_stripe === "trialing" && stripeInfo?.trial_end;
  const diasTrial = esTrial
    ? Math.ceil(
        (new Date(stripeInfo.trial_end) - new Date()) / (1000 * 60 * 60 * 24),
      )
    : null;
  ...
  if (facturasAbiertas.length > 0 && suscripcion.fecha_fin) {
    ...
    fechaCorteAviso = new Date(
      fin.getFullYear(),
      fin.getMonth() + MESES_TOLERANCIA + 1,
      1,
    );
  }
    ...
    <div className="space-y-6 max-w-2xl">
        ...
        <div className="flex items-start gap-3 bg-blue-50 border border-blue-300 rounded-xl px-4 py-4 text-sm text-blue-800">
          ...
          <div>
            ...
            <p className="mt-0.5 text-blue-700">
              {diasTrial > 0
                ? `Te quedan ${diasTrial} día${
                    diasTrial === 1 ? "" : "s"
                  } de prueba gratis. Al vencerse, se realizará el primer cobro automático.`
          ...
          <div className="flex-1">
            ...
            <p className="mt-0.5 text-orange-700">
              {diasParaCorte != null && diasParaCorte > 0
                ? `Tu servicio se suspenderá en ${diasParaCorte} día${
                    diasParaCorte === 1 ? "" : "s"
                  } (${formatDate(
                    fechaCorteAviso,
      ...
      <div className="border rounded-xl p-6 space-y-4">
        <div className="flex items-start justify-between gap-4">
          ...
          <span
            className={`flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-full ${estadoInfo.color}`}
          >
        ...
        <div className="grid grid-cols-2 gap-4 text-sm">
          <InfoRow label="Precio mensual">
            {precioMensualFinal > 0
              ? `${formatMXN(precioMensualFinal)} / mes`
              : "—"}
          </InfoRow>
          <InfoRow label="Próximo cobro">
            {stripeInfo?.proximo_cobro
              ? formatDate(stripeInfo.proximo_cobro)
              : formatDate(
                  suscripcion.fecha_fin || suscripcion.fecha_vencimiento,
                )}
          </InfoRow>
          ...
          <InfoRow label="Meses pagados">
            {mesesPagados > 0
              ? `${mesesPagados} ${mesesPagados === 1 ? "mes" : "meses"}`
              : "—"}
          </InfoRow>
        </div>
        ...
        {stripeInfo?.tarjeta && (
          <div className="flex items-center gap-3 bg-slate-50 border border-slate-200 rounded-lg px-4 py-3 text-sm">
            ...
            <span className="text-slate-700 capitalize">
              {stripeInfo.tarjeta.marca}
            </span>
            <span className="text-slate-500">
              •••• {stripeInfo.tarjeta.ultimos4}
            </span>
            <span className="text-slate-400 text-xs ml-auto">
              Vence {stripeInfo.tarjeta.expira}
            </span>
          </div>
        )}
        ...
        {!suscripcion.stripe_customer_id && (
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 text-sm text-blue-800">
            ...
            <p className="mb-3">
              Tu contrato está activo pero aún no tienes pago automático
              configurado. Al activarlo se te cobrará de forma mensual el día 1
              de cada mes.
            </p>
            <Button
              size="sm"
        ...
        {suscripcion.stripe_customer_id &&
          ...
          suscripcion.estado_suscripcion !== "Vencida" && (
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 text-sm text-yellow-800">
              ...
              <p className="mb-3">
                Aún no has completado el proceso de pago. Si el enlace expiró,
                genera uno nuevo.
              </p>
              <div className="flex flex-wrap gap-2">
                {suscripcion.enlace_pago_stripe && (
                  <a
                    href={suscripcion.enlace_pago_stripe}
                    target="_blank"
                    rel="noreferrer"
          ...
          suscripcion.stripe_customer_id && (
            <div className="space-y-2">
              ...
              >
                ...
                {abriendo
                  ? "Abriendo portal..."
                  : "Cambiar tarjeta o cancelar suscripción"}
              ...
              {!suscripcion.stripe_subscription_id && (
                <p className="text-xs text-slate-400 text-center">
                  ...
                  >
                    {generando
                      ? "Generando..."
                      : "Haz clic aquí para activar el cobro automático"}
                  </button>
                </p>
              )}
            </div>
              ...
              planesData.planes.map((plan) => {
                const nombre =
                  plan.usuarios_min != null && plan.usuarios_max != null
                    ...
                    : `Plan #${plan.id}`;
                const costoUsuario = Number(
                  suscripcion.precio_empleado_extra || 60,
                );
                const empleadosPlan = Number(plan.usuarios_max || 0);
                const precio = empleadosPlan * costoUsuario;
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-slate-400 ml-auto">
              {mesesPagados}{" "}
              {mesesPagados === 1 ? "mes pagado" : "meses pagados"}
            </span>
          </div>
          <div className="border rounded-xl overflow-hidden divide-y">
            {stripeInfo.facturas.map((factura) => {
              const estadoF = ESTADO_FACTURA[factura.estado] || {
                ...
                >
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-slate-400 mt-0.5 capitalize">
                      {formatMes(factura.periodo_inicio)} ·{" "}
                      {formatDate(factura.fecha)}
                    </p>
                  </div>
                  <span
                    className={`text-xs font-semibold px-2.5 py-1 rounded-full shrink-0 ${estadoF.color}`}
                  >
                    {estadoF.label}
                  </span>
                  {factura.pdf_url && (
